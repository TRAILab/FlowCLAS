"""Data module for Fishyscapes dataset."""
...
class FishyscapesDataset(Dataset):
    """Fishyscapes validation images with optional SAM/CLIP auxiliary tensors."""
    ...
    def __init__(self, 
        ...
        self.images = [str(image) for image in self.root.glob("images/*.png")]
        self.targets = [str(image).replace("images", "labels") for image in self.images]

    def __len__(self) -> int:
        return len(self.images)

    def __getitem__(self, index: int) -> dict[str, str | torch.Tensor]:
        """Return paths, transformed tensors, and ``original_shape`` for ``index``."""
...
class Fishyscapes(BaseDataModule):
    """Data module for Fishyscapes dataset."""
    def __init__(
        self,
        root: Path | str = "fishyscapes",
